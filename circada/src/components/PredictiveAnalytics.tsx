          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="text-center p-4 bg-zinc-900/50 rounded-lg">
                <div className="text-2xl font-bold !text-white">{heartRate} bpm</div>
                <div className="text-sm !text-zinc-400">Current Heart Rate</div>
                <Badge className={`mt-2 ${hrVariance.trend === 'increasing' ? '!bg-red-500/20 !text-red-300' : 
                  hrVariance.trend === 'decreasing' ? '!bg-blue-500/20 !text-blue-300' : 
              ...
              <div className="text-center p-4 bg-zinc-900/50 rounded-lg">
                ...
                <div className="text-sm !text-zinc-400">Predicted Peaks Today</div>
                <div className="text-xs !text-zinc-500 mt-1">Based on your pattern</div>
              </div>
              ...
              <div className="text-center p-4 bg-zinc-900/50 rounded-lg">
                ...
                <div className="text-xs !text-zinc-500 mt-1">Last 7 days average</div>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            {predictions.map((prediction, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-zinc-900/50 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-medium !text-white">{prediction.time} - {prediction.description}</div>
                    <div className="text-sm !text-zinc-400">{prediction.recommendation}</div>
                  </div>
                </div>
            ...
            <svg width="100%" height="100%" viewBox="0 0 400 120" className="overflow-visible">
              ...
              {energyPredictions.filter((_, i) => i % 2 === 0).map((p, i) => (
                <text key={i} x={(i * 2) / (energyPredictions.length - 1) * 400} y="115" 
                      textAnchor="middle" className="fill-zinc-400 text-xs">
                  {p.label}
                </text>
              ))}
            </svg>
          ...
          <CardContent>
            <div className="space-y-3 text-sm">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="!text-white font-medium">Your typical wake time: {Math.floor(realDataAnalysis.personalizedWakeTime)}:{String(Math.round((realDataAnalysis.personalizedWakeTime % 1) * 60)).padStart(2, '0')}</div>
                  <div className="!text-zinc-400">Based on your sleep pattern history</div>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="!text-white font-medium">Your energy peaks occur around: {realDataAnalysis.energyPeaks?.map((p: number) => `${Math.floor(p)}:${String(Math.round((p % 1) * 60)).padStart(2, '0')}`).join(', ')}</div>
                  <div className="!text-zinc-400">Schedule demanding tasks during these windows</div>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="!text-white font-medium">Your sleep efficiency is {Math.round(realDataAnalysis.sleepEfficiency * 100)}%</div>
                  <div className="!text-zinc-400">{realDataAnalysis.sleepEfficiency > 0.85 ? 'Excellent sleep quality' : 'Room for improvement in sleep optimization'}</div>
                </div>
              </div>
            </div>
          </CardContent>
